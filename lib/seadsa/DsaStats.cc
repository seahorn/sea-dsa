...
static llvm::cl::opt<unsigned> TypeDisparitySummarySize(
    ...
    llvm::cl::desc("Maximum number of allocation sites shown in the type-disparity summary"),
    llvm::cl::Hidden,
    llvm::cl::init(20));
...
namespace seadsa {
  ...
  static void printGraphInfo(nodes_range nodes, llvm::raw_ostream &o) {
    ...
    if (total_nodes > 0) {
      o << "\t" << (double)num_links/(double)total_nodes << " average number of node links (graph fan-out)\n";
  ...
  static void printAllocInfo(nodes_range nodes,
    ...
    unsigned total_num_alloc_sites = std::distance(alloc_sites.begin(), alloc_sites.end());
    // to print type disparity in allocation sizes
    std::vector<std::pair<const NodeInfo*,  SmallPtrSet<Type*, 32>>> typeDisparity;
    ...
    for (const auto &n: nodes) {
      ...
      typeDisparity.push_back(std::pair<const NodeInfo*,  SmallPtrSet<Type*, 32>>(&n, allocTypes));
    }
    ...
    for (unsigned i=0; i < TypeDisparitySummarySize; ++i) {
      ...
      o << "\t\t [Node Id " << nodei->getId() << "] IsCollapsed=" << nodei->getNode()->isOffsetCollapsed() 
