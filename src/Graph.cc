...
void sea_dsa::Node::addAllocSite(const Value& v) 
{
...
bool sea_dsa::Graph::IsGlobal::operator() (const ValueMap::value_type &kv) const
...
sea_dsa::Cell &sea_dsa::Graph::mkRetCell (const llvm::Function &fn, const Cell &c)
{
...
const sea_dsa::Cell &sea_dsa::Graph::getRetCell (const llvm::Function &fn) const
{
  auto it = m_returns.find (&fn);
  assert (it != m_returns.end ());
  return *(it->second);
...
bool sea_dsa::Graph::computeCalleeCallerMapping (const DsaCallSite &cs, 
