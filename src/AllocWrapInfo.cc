...
ExtraAllocs("alloc-function",
            llvm::cl::desc("malloc-like functions"),
            llvm::cl::ZeroOrMore);

static llvm::cl::list<std::string>
ExtraFrees("dealloc-function",
            llvm::cl::desc("free-like functions"),
            llvm::cl::ZeroOrMore);

namespace {bool isNotStored(Value *V);}
...
void AllocWrapInfo::findAllocs(Module &M) {
    for (Function &F : M) {
        if (m_allocs.count(F.getName())) continue;
...
bool AllocWrapInfo::findWrappers(Module &M, std::set<std::string> &fn_names) {
    ...
    for (auto &n : fns) {
        ...
        for (User *U : F->users()) {
            ...
            if (fn_names.count(parentFn->getName())) continue;
            if (parentFn->doesNotReturn()) continue;
            if (!parentFn->getReturnType()->isPointerTy()) continue;
            ...
            if (isWrapper) isWrapper &= isNotStored(CI);
            if (isWrapper) {
...
bool AllocWrapInfo::flowsFrom(Value *dst, Value *src) {
    if (dst == src) return true;
        ...
        if(LI.isLoopHeader(PN->getParent())) return false;
